...
contract TestUtils is Test {
    ...
    constructor() {
        ...
        vm.setEnv("LOCAL_PRIVATE_KEY", "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80");

        //
        _NONCE = uint256(
            keccak256(
                abi.encode(tx.origin, tx.origin.balance, block.number, block.timestamp, block.coinbase, gasleft()) //solhint-disable-line
                    // avoid-tx-origin
            )
        );
    }
    ...
    function _randomAddress(uint256 amount) internal view returns (address[] memory) {
